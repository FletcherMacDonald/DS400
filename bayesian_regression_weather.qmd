...
```{r}
# Check unique locations in the data
unique(weather_WU$location)
table(weather_WU$location)
```

```{r}
# Compare morning vs afternoon temperatures for both locations
ggplot(weather_WU, aes(x = temp9am, y = temp3pm, color = location)) + 
  geom_point(alpha = 0.7, size = 2) + 
  geom_smooth(method = "lm", se = TRUE, size = 1.2) +
  labs(title = "Morning vs Afternoon Temperature Comparison",
       subtitle = "Uluru (Desert) vs Wollongong (Coastal) in same plot",
...
We simulate the model posterior below. This model uses a simple linear relationship between morning and afternoon temperatures.
...
Given the useful information it provides, we're tempted to incorporate `location` into our temperature analysis. We can! This model will help us understand how location affects afternoon temperature independently of morning temperature.
...
## Model 3: Two Predictors

Now let's combine both predictors to see how morning temperature and location together affect afternoon temperature.
...
Finally, let's include all available predictors to see if we can improve our predictions further by incorporating windspeed, humidity, and pressure measurements from 9am.
