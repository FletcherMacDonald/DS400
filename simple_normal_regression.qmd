...
ggplot(bikes, aes(x = temp_feel, y = rides)) +
  ...
  stat_cor() +
  labs(title = "Capital Bikeshare: Temperature vs Ridership",
       subtitle = "Relationship between temperature and daily rides",
       x = "Temperature (°F)",
       y = "Number of Rides",
...
```{r}
# Enhanced visualization with correlation and summary stats
# Calculate correlation
correlation <- cor(bikes$temp_feel, bikes$rides)

# Create comprehensive plot
ggplot(bikes, aes(x = temp_feel, y = rides)) +
  geom_point(alpha = 0.7, color = "darkblue", size = 1.5) +
  geom_smooth(method = "lm", se = TRUE, color = "red", size = 1.2) +
  geom_vline(xintercept = 75, linetype = "dashed", color = "green", size = 1) +
  annotate("text", x = 75, y = max(bikes$rides) * 0.9, 
           label = "75°F", color = "green", size = 4, fontface = "bold") +
  labs(title = paste("Temperature vs Ridership (r =", round(correlation, 3), ")"),
       subtitle = "Strong positive correlation between temperature and ridership",
       x = "Temperature (°F)",
       y = "Number of Rides") +
  theme_minimal() +
  theme(plot.title = element_text(size = 14, face = "bold"))
...
cat("Mean rides at 75°F (from data near 75°F):", 
    round(mean(bikes$rides[bikes$temp_feel >= 70 & bikes$temp_feel <= 80]), 0), "\n")
cat("Range of rides at 75°F:", 
    round(min(bikes$rides[bikes$temp_feel >= 70 & bikes$temp_feel <= 80]), 0), "-",
    round(max(bikes$rides[bikes$temp_feel >= 70 & bikes$temp_feel <= 80]), 0), "\n")
```
...
mcmc_trace(bike_model, pars = c("(Intercept)", "temp_feel", "sigma"))
```
...
pp_check(bike_model, nreps = 50) +
  labs(title = "Posterior Predictive Check",
       subtitle = "Comparing observed data (dark blue) with simulated data (light blue)")
```

```{r}
# Residual analysis
residuals <- residuals(bike_model)
ggplot(data.frame(residuals = residuals, temp_feel = bikes$temp_feel), 
       aes(x = temp_feel, y = residuals)) +
...
The prediction interval of **1,492 to 6,503 rides** for a 75°F day reflects several sources of uncertainty:

1.  **Model Uncertainty**: Our linear model assumes a constant relationship between temperature and ridership, but real relationships might be more complex.

2.  **Parameter Uncertainty**: Even with our priors and data, we're not 100% certain about the exact slope and intercept.

3.  **Natural Variability**: Even at the same temperature, ridership varies due to factors not in our model (weather, events, day of week, etc.).

4.  **Prediction vs. Estimation**: We're predicting a single day's ridership, which is inherently more uncertain than estimating the average relationship.

**Key Takeaway**: The wide prediction interval (about 5,000 rides) shows that while temperature is a strong predictor, it's not the only factor driving ridership. For business planning, Capital Bikeshare should prepare for this range of possible outcomes.
