...
```{r}
plot_gamma(10, 2, mean = TRUE, mode = TRUE)
```
...
```{r}
plot_gamma(shape = 7/3, rate = 1/3, mean = TRUE, mode = TRUE)
```

1.  The most common value of λ is 10 and the mean is 12.

```{r}
plot_gamma(shape = 12/2, rate = 1/2, mean = TRUE, mode = TRUE)
```
...
```{r}
plot_gamma(shape = 1, rate = 1/4, mean = TRUE, mode = TRUE)
```

1.  Why is the Poisson model a reasonable choice for our data Yi?

    It is count data, so it fits the data type of the Poisson model better. Also, generally fits our prior understanding
...
```{r}
summarize_gamma_poisson(1,.25, sum_y = 146, n=52)
```

```{r}
plot_gamma_poisson(1,.25, sum_y = 146, n=52)
```
