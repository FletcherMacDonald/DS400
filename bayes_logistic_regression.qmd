...
```{r}
 is_cancelled <- stan_glm(raintomorrow ~ humidity9am,
...
```{r}
exp(posterior_interval(is_cancelled, prob = 0.80))
```
...
```{r}
weather %>%
  add_fitted_draws(is_cancelled, n = 100) %>%
...
```{r}
classification_summary(model = is_cancelled, data = weather, cutoff = 0.5)
```
...
```{r}
classification_summary(model = is_cancelled, data = weather, cutoff = 0.2)
```
...
## Exercise 13.6 Solutions

### 1. Proportion of canceled bookings
...
```{r}
# Plot 1: is_canceled vs lead_time
ggplot(hotel_bookings, aes(x = lead_time, fill = is_canceled)) +
...
```{r}
# Plot 2: is_canceled vs previous_cancellations
ggplot(hotel_bookings, aes(x = previous_cancellations, fill = is_canceled)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "Previous Cancellations vs Current Cancellation Status",
       x = "Number of Previous Cancellations", y = "Count") +
  theme_minimal()
```

```{r}
# Plot 3: is_canceled vs is_repeated_guest
ggplot(hotel_bookings, aes(x = is_repeated_guest, fill = is_canceled)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "Repeat Guest Status vs Cancellation",
       x = "Is Repeated Guest", y = "Count") +
  theme_minimal()
```

```{r}
# Plot 4: is_canceled vs average_daily_rate
ggplot(hotel_bookings, aes(x = average_daily_rate, fill = is_canceled)) +
  geom_histogram(bins = 30, alpha = 0.7) +
  labs(title = "Average Daily Rate vs Cancellation Status",
...
### 4. Interpretation of Key Predictors

```{r}
exp(posterior_interval(hotel_model, prob = 0.80))
```

#### **lead_time** (per 1 day increase)

-   **OR = 1.002 to 1.003** → **+0.2% to +0.3%** change in odds per day
-   Small but consistent positive effect: longer lead times are associated with slightly higher cancellation odds

#### **previous_cancellations** (per 1 additional cancellation)

-   **OR = 1.15 to 1.18** → **+15% to +18%** higher odds per additional previous cancellation
-   Strong positive effect: guests with more cancellation history are much more likely to cancel again

#### **is_repeated_guest = 1 (vs 0)**

-   **OR = 0.45 to 0.50** → **-50% to -55%** lower odds for repeat guests
-   Strong negative effect: repeat guests are much less likely to cancel

#### **average_daily_rate** (per 1 unit increase)

-   **OR = 0.999 to 1.000** → **-0.1% to 0%** change in odds per unit
-   Minimal effect: price has little impact on cancellation probability

### 5. Model Accuracy Analysis

```{r}
# Default cutoff (0.5)
classification_summary(model = hotel_model, data = hotel_bookings, cutoff = 0.5)
```

```{r}
# Lower cutoff (0.3) - more sensitive to cancellations
classification_summary(model = hotel_model, data = hotel_bookings, cutoff = 0.3)
```

```{r}
# Higher cutoff (0.7) - more conservative
classification_summary(model = hotel_model, data = hotel_bookings, cutoff = 0.7)
```

### Key Findings:

1.  **Previous cancellation history** is the strongest predictor - guests who have canceled before are much more likely to cancel again
2.  **Repeat guests** are significantly less likely to cancel, showing loyalty effects
3.  **Lead time** has a small but consistent positive effect on cancellation probability
4.  **Price (average daily rate)** has minimal impact on cancellation decisions
5.  The model performs reasonably well, with the default cutoff providing good overall accuracy, though there's a trade-off between sensitivity and specificity when adjusting the cutoff
