...
```{r}
# Explore dataset and select predictors
glimpse(pulse_of_the_nation)
pulse_of_the_nation %>% tabyl(climate_change)

# Check if transformers and income exist - if so, use them; otherwise use age, education, political variable
if("transformers" %in% names(pulse_of_the_nation) && "income" %in% names(pulse_of_the_nation)) {
  # Use transformers and income
  predictor_vars <- c("climate_change", "transformers", "income")
  pulse_clean <- pulse_of_the_nation %>% 
    select(all_of(predictor_vars)) %>% 
    filter(!is.na(climate_change), !is.na(transformers), !is.na(income))
  cat("Using predictors: transformers, income\n")
} else {
  # Use age, education, and political variable (original approach)
  ...
  cat("Using predictors: age, education,", political_var_name, "\n")
}
...
```{r}
# Build model - use transformers and income if available, otherwise use age, education, political_var
if("transformers" %in% names(pulse_clean) && "income" %in% names(pulse_clean)) {
  climate_model <- naiveBayes(climate_change ~ transformers + income, data = pulse_clean)
  climate_model
  
  # Create test person with transformers and income
  ...
  # Create test person with age, education, political_var
  test_person <- data.frame(
    age = 45,
...
**Note:** If you want to use different predictors (like `transformers` and `income`), first check if they exist:

```{r}
# Check if transformers and income exist in the dataset
cat("Checking for variables:\n")
cat("transformers exists:", "transformers" %in% names(pulse_of_the_nation), "\n")
cat("income exists:", "income" %in% names(pulse_of_the_nation), "\n")

# If they exist, show their structure
if("transformers" %in% names(pulse_of_the_nation) && "income" %in% names(pulse_of_the_nation)) {
  cat("\nVariable types:\n")
  cat("transformers:", class(pulse_of_the_nation$transformers), "\n")
  cat("income:", class(pulse_of_the_nation$income), "\n")
  cat("\nSample values:\n")
  head(pulse_of_the_nation %>% select(transformers, income, climate_change))
} else {
  cat("\nThese variables don't exist. Available variables:\n")
  print(names(pulse_of_the_nation))
}
```

If `transformers` and `income` exist, use this syntax:
...
**Note:** These calculations use the predictors from your model. If you used `transformers` and `income`, adjust the code accordingly.
...
# Initialize all likelihoods to 1 (will multiply by actual likelihoods)
likelihood_quant1_not_real <- 1
likelihood_quant1_not_caused <- 1
likelihood_quant1_caused_by_people <- 1

likelihood_quant2_not_real <- 1
likelihood_quant2_not_caused <- 1
likelihood_quant2_caused_by_people <- 1

likelihood_education_not_real <- 1
likelihood_education_not_caused <- 1
likelihood_education_caused_by_people <- 1

likelihood_political_not_real <- 1
likelihood_political_not_caused <- 1
likelihood_political_caused_by_people <- 1

# Step 2: Quantitative predictor 1 (transformers or age)
if("transformers" %in% names(pulse_clean) && "transformers" %in% names(test_person)) {
  transformers_stats <- pulse_clean %>% group_by(climate_change) %>% summarize(mean_transformers = mean(transformers, na.rm = TRUE), sd_transformers = sd(transformers, na.rm = TRUE))
  test_transformers <- as.numeric(test_person$transformers[1])
  likelihood_quant1_not_real <- dnorm(test_transformers, mean = transformers_stats$mean_transformers[transformers_stats$climate_change == "Not Real at All"], 
                                      sd = transformers_stats$sd_transformers[transformers_stats$climate_change == "Not Real at All"])
  likelihood_quant1_not_caused <- dnorm(test_transformers, mean = transformers_stats$mean_transformers[transformers_stats$climate_change == "Real but not Caused by People"], 
                                       sd = transformers_stats$sd_transformers[transformers_stats$climate_change == "Real but not Caused by People"])
  likelihood_quant1_caused_by_people <- dnorm(test_transformers, mean = transformers_stats$mean_transformers[transformers_stats$climate_change == "Real and Caused by People"], 
                                              sd = transformers_stats$sd_transformers[transformers_stats$climate_change == "Real and Caused by People"])
} else if("age" %in% names(pulse_clean) && "age" %in% names(test_person)) {
  age_stats <- pulse_clean %>% group_by(climate_change) %>% summarize(mean_age = mean(age, na.rm = TRUE), sd_age = sd(age, na.rm = TRUE))
  test_age <- as.numeric(test_person$age[1])
  likelihood_quant1_not_real <- dnorm(test_age, mean = age_stats$mean_age[age_stats$climate_change == "Not Real at All"], 
                                      sd = age_stats$sd_age[age_stats$climate_change == "Not Real at All"])
  likelihood_quant1_not_caused <- dnorm(test_age, mean = age_stats$mean_age[age_stats$climate_change == "Real but not Caused by People"], 
                                       sd = age_stats$sd_age[age_stats$climate_change == "Real but not Caused by People"])
  likelihood_quant1_caused_by_people <- dnorm(test_age, mean = age_stats$mean_age[age_stats$climate_change == "Real and Caused by People"], 
                                              sd = age_stats$sd_age[age_stats$climate_change == "Real and Caused by People"])
}

# Step 3: Quantitative predictor 2 (income)
if("income" %in% names(pulse_clean) && "income" %in% names(test_person)) {
  income_stats <- pulse_clean %>% group_by(climate_change) %>% summarize(mean_income = mean(income, na.rm = TRUE), sd_income = sd(income, na.rm = TRUE))
  test_income <- as.numeric(test_person$income[1])
  likelihood_quant2_not_real <- dnorm(test_income, mean = income_stats$mean_income[income_stats$climate_change == "Not Real at All"], 
                                     sd = income_stats$sd_income[income_stats$climate_change == "Not Real at All"])
  likelihood_quant2_not_caused <- dnorm(test_income, mean = income_stats$mean_income[income_stats$climate_change == "Real but not Caused by People"], 
                                        sd = income_stats$sd_income[income_stats$climate_change == "Real but not Caused by People"])
  likelihood_quant2_caused_by_people <- dnorm(test_income, mean = income_stats$mean_income[income_stats$climate_change == "Real and Caused by People"], 
                                             sd = income_stats$sd_income[income_stats$climate_change == "Real and Caused by People"])
}

# Step 4: Categorical predictor - Education
if("education" %in% names(pulse_clean) && "education" %in% names(test_person)) {
  test_education <- as.character(test_person$education[1])
  education_counts <- pulse_clean %>% 
    group_by(climate_change, education) %>% 
    summarize(n = n(), .groups = "drop") %>%
    group_by(climate_change) %>%
    mutate(total = sum(n), prob = n / total)
  
  likelihood_education_not_real <- education_counts$prob[education_counts$climate_change == "Not Real at All" & education_counts$education == test_education]
  if(length(likelihood_education_not_real) == 0) likelihood_education_not_real <- 0
  
  likelihood_education_not_caused <- education_counts$prob[education_counts$climate_change == "Real but not Caused by People" & education_counts$education == test_education]
  if(length(likelihood_education_not_caused) == 0) likelihood_education_not_caused <- 0
  
  likelihood_education_caused_by_people <- education_counts$prob[education_counts$climate_change == "Real and Caused by People" & education_counts$education == test_education]
  if(length(likelihood_education_caused_by_people) == 0) likelihood_education_caused_by_people <- 0
}

# Step 5: Categorical predictor - Political variable
if("political_var" %in% names(pulse_clean) && "political_var" %in% names(test_person)) {
  test_political <- as.character(test_person$political_var[1])
  political_counts <- pulse_clean %>% 
    group_by(climate_change, political_var) %>% 
    summarize(n = n(), .groups = "drop") %>%
    group_by(climate_change) %>%
    mutate(total = sum(n), prob = n / total)
  
  likelihood_political_not_real <- political_counts$prob[political_counts$climate_change == "Not Real at All" & political_counts$political_var == test_political]
  if(length(likelihood_political_not_real) == 0) likelihood_political_not_real <- 0
  
  likelihood_political_not_caused <- political_counts$prob[political_counts$climate_change == "Real but not Caused by People" & political_counts$political_var == test_political]
  if(length(likelihood_political_not_caused) == 0) likelihood_political_not_caused <- 0
  
  likelihood_political_caused_by_people <- political_counts$prob[political_counts$climate_change == "Real and Caused by People" & political_counts$political_var == test_political]
  if(length(likelihood_political_caused_by_people) == 0) likelihood_political_caused_by_people <- 0
}

# Step 6: Calculate unnormalized posteriors (prior * all likelihoods)
unnorm_posterior_not_real <- prior_prob_not_real * likelihood_quant1_not_real * likelihood_quant2_not_real * 
  likelihood_education_not_real * likelihood_political_not_real

unnorm_posterior_not_caused <- prior_prob_not_caused * likelihood_quant1_not_caused * likelihood_quant2_not_caused * 
  likelihood_education_not_caused * likelihood_political_not_caused

unnorm_posterior_caused_by_people <- prior_prob_caused_by_people * likelihood_quant1_caused_by_people * likelihood_quant2_caused_by_people * 
  likelihood_education_caused_by_people * likelihood_political_caused_by_people

# Step 7: Normalizing constant (sum of unnormalized posteriors)
normalizing_constant <- unnorm_posterior_not_real + unnorm_posterior_not_caused + unnorm_posterior_caused_by_people

# Step 8: Normalized posterior probabilities
posterior_not_real <- unnorm_posterior_not_real / normalizing_constant
posterior_not_caused <- unnorm_posterior_not_caused / normalizing_constant
posterior_caused_by_people <- unnorm_posterior_caused_by_people / normalizing_constant

# Display results
cat("Posterior Probabilities:\n")
cat("Not Real at All:", round(posterior_not_real, 4), "\n")
cat("Real but not Caused by People:", round(posterior_not_caused, 4), "\n")
cat("Real and Caused by People:", round(posterior_caused_by_people, 4), "\n")
cat("Sum:", round(posterior_not_real + posterior_not_caused + posterior_caused_by_people, 4), "\n")
```
